#!/usr/bin/env python3
"""
Phase 2: Lyric Line to Word Sequence Conversion (Fixed)
Using correct property names and AuraDB patterns
"""
...
def convert_lyric_lines_to_sequences():
    """Convert LyricLine nodes to word ID sequences using correct property names"""
    ...
    try:
        with driver.session() as session:
            ...
            total_query = "MATCH (l:LyricLine) WHERE l.text IS NOT NULL RETURN count(l) as total"
            total_lines = session.run(total_query).single()["total"]
            logger.info(f"📊 Processing {total_lines:,} lyric lines with text...")
            
            # Step 2: Convert LyricLines to word sequences using correct property
            conversion_query = """
            ...
            SET l.word_sequence = word_ids,
                ...
                l.system_version = 'phase2_fixed_v1.0'
            ...
            SET s.total_word_count = size(all_word_ids),
                ...
                s.word_system_version = 'phase2_fixed_v1.0'
            ...
            if stats['avg_complexity'] is not None:
                ...
                logger.info("   No vocabulary statistics available")
            
            return {
                'lines_converted': lines_converted,
                'songs_updated': songs_updated,
                'validation_stats': dict(stats) if stats else {}
            }
...
if __name__ == "__main__":
    print("🚀 Starting Phase 2: Lyric Line to Word Sequence Conversion (Fixed)")
    print("=" * 65)
