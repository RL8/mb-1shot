...
class Phase1TaxonomyCalculator:
    ...
    def calculate_danceability_enhanced(self, danceability: float, tempo: float, energy: float) -> Tuple[float, str]:
        """5. Enhanced Danceability calculation"""
        # Use Spotify's danceability but enhance with tempo and energy validation
        tempo_factor = 1.0 if 90 <= tempo <= 150 else 0.8  # Optimal dance tempo range
        energy_factor = min(energy * 1.2, 1.0)  # Boost for higher energy
        
        score = danceability * tempo_factor * energy_factor
        
        # Determine label
        if score < 0.3:
            label = "Not Danceable"
        elif score < 0.5:
            label = "Slightly Danceable"
        elif score < 0.7:
            label = "Moderate Dance"
        else:
            label = "Very Danceable"
    ...
    def calculate_nostalgia_factor(self, release_year: int, genre_primary: str = "", user_age: int = 25) -> Tuple[float, str]:
        """7. Nostalgia Factor calculation (simplified for Phase 1)"""
        current_year = 2024
        song_age = current_year - release_year
        
        # Peak nostalgia typically occurs 10-20 years after release
        if 10 <= song_age <= 20:
            age_factor = 0.8
        elif 5 <= song_age <= 25:
            age_factor = 0.6
        ...
        # Genre era matching (simplified)
        genre_factor = 0.5  # Default neutral value for Phase 1
        
        score = age_factor * 0.7 + genre_factor * 0.3
        
        # Determine label and era
        if song_age <= 5:
            label = "Recent"
        elif song_age <= 10:
            label = "Nostalgic"
        elif song_age <= 20:
            label = "Very Nostalgic"
        else:
            label = "Classic"
    ...
    def determine_time_of_day(self, energy: float, tempo: float, acousticness: float, valence: float, danceability: float) -> Tuple[str, str]:
        ...
        # Morning Reflection
        if energy < 0.5 and valence < 0.5 and acousticness > 0.4:
            return "Morning Reflection", "Morning"
        
        return "Any Time", "Any Time"
    
    def determine_activity_match(self, tempo: float, energy: float, danceability: float, instrumentalness: float, valence: float) -> Tuple[str, str]:
        ...
        # Meditation/Yoga
        if energy < 0.3 and valence > 0.3 and acousticness > 0.5:
            return "Meditation", "Relaxation"
        
        return "General Listening", "General"
    
    def calculate_genre_fluidity(self, genre_primary: str = "", genre_secondary: str = "", cross_genre_elements: int = 1) -> Tuple[float, str]:
        """12. Genre Fluidity calculation (simplified for Phase 1)"""
        # Base score from number of genres
        genre_count_factor = 0 if not genre_secondary else 0.3
        
        # Cross-genre elements (simplified)
        cross_genre_factor = min(cross_genre_elements * 0.2, 0.6)
        
        # Innovation index (simplified - could be enhanced with actual genre analysis)
        innovation_factor = 0.1  # Default low innovation for Phase 1
        
        score = genre_count_factor + cross_genre_factor + innovation_factor
        
        # Determine label
        if score < 0.2:
            label = "Pure Genre"
        elif score < 0.4:
            label = "Slightly Mixed"
        elif score < 0.6:
            label = "Fusion"
        else:
            label = "Highly Fluid"
    ...
    def calculate_taxonomies_for_song(self, song: Dict) -> Dict:
        ...
        taxonomies['danceability_enhanced'], taxonomies['dance_label'] = self.calculate_danceability_enhanced(danceability, tempo, energy)
        taxonomies['focus_suitability'], taxonomies['focus_label'] = self.calculate_focus_suitability(instrumentalness, speechiness, energy)
        taxonomies['nostalgia_factor'], taxonomies['nostalgia_label'] = self.calculate_nostalgia_factor(2020)  # Default to recent for Phase 1
        taxonomies['social_context'], taxonomies['social_label'] = self.calculate_social_context(energy, valence, 50, danceability)  # Default popularity
        ...
        taxonomies['activity_match'], taxonomies['activity_category'] = self.determine_activity_match(tempo, energy, danceability, instrumentalness, valence)
        taxonomies['genre_fluidity'], taxonomies['fluidity_label'] = self.calculate_genre_fluidity()
    ...
    def process_all_songs(self, batch_size: int = 50) -> Dict:
